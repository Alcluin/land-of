			...
			for (var i = 0; i < m_Mobiles.Length; ++i)
			{
				...
				if (m != null && !m.Deleted)
				{
					...
					xml.WriteString(m.Serial.Value.ToString());
						//accounting edit by Phoo to add character names to account xml
						//xml.WriteStartElement("CharacterName:");
						//xml.WriteString(m.Name);
						//xml.WriteEndElement();
						//end of Phoo edit to add character names to account xml
					xml.WriteEndElement();
				}
			}
