using System;
using System.Collections; // OWLTR
...
using Server.Items; // for Kudos
using Server.Custom.GameTimeReward; // for Kudos
...
namespace Server.Engines.CannedEvil
{
    ...
    public class IdolOfTheChampion : Item
    {
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (this == null || this.Deleted || this.Spawn == null || this.Spawn.Deleted)
            ...
            else if (this.Spawn.HasBeenAdvanced)
                from.SendLocalizedMessage(1054038); // The Champion of this region has already been challenged!
			else
			{
				Item[] item = from.Backpack.FindItemsByType( typeof( Kudos ) );
				if ( from.Backpack.ConsumeTotal( typeof( Kudos ), 60 ) )
				{
					from.SendMessage( "60 Kudos has been removed from your pack." );
					from.SendLocalizedMessage(1054037); // Your challenge is heard by the Champion of this region! Beware its wrath!
					this.Spawn.EndRestart();
					this.Spawn.HasBeenAdvanced = true;
					
					return;
				}
				
				Container bank = from.FindBankNoCreate();
				bank.FindItemsByType( typeof( Kudos ) );
				if( bank.ConsumeTotal( typeof( Kudos ), 60 ) )
				{
					from.SendMessage( "60 Kudos has been removed from your bank." );
					from.SendLocalizedMessage(1054037); // Your challenge is heard by the Champion of this region! Beware its wrath!
					this.Spawn.EndRestart();
					this.Spawn.HasBeenAdvanced = true;
				...
				{
					from.SendMessage("To summon a Champion you need 60 Kudos in your backpack or bank");
				}
			}
			// Container bank = from.FindBankNoCreate();
			
			// if( !SearchPackForItemType(from.Backpack, Kudos) || !SearchPackForItemType(bank, Kudos) )
			// {
				// from.SendMessage("To summon a Champion you need 60 Kudos in your backpack or bank");
				// return;
			// }
